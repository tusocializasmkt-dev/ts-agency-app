...
export default function AgencySettings() {
  ...
  const handleSave = async () => { try { validatePaymentSettings(config); } catch (error) { feedback.error((error as Error).message); return; } try { await save(config); feedback.success('Configurações salvas!'); } catch { feedback.error('Erro ao salvar'); } };
  ...
  return <div className="max-w-4xl space-y-8"><div className="space-y-10 rounded-[2.5rem] border border-zinc-200 bg-white p-5 shadow-sm sm:p-10">
    ...
    <div className="grid gap-12 md:grid-cols-2"><div className="space-y-8"><Input label="Nome da Agência" value={config.name} onChange={value => setConfig({ ...config, name: value })} /><Input label="E-mail Corporativo" value={config.email} onChange={value => setConfig({ ...config, email: value })} /><Input label="Telefone / WhatsApp" value={config.phone} onChange={value => setConfig({ ...config, phone: value })} /></div><div className="space-y-8 rounded-[2rem] border bg-zinc-50/50 p-8"><h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-400">Links Sociais</h3><SocialInput icon={Instagram} label="Instagram" value={config.socialLinks?.instagram || ''} onChange={value => setConfig({ ...config, socialLinks: { ...config.socialLinks, instagram: value } })} /><SocialInput icon={Facebook} label="Facebook" value={config.socialLinks?.facebook || ''} onChange={value => setConfig({ ...config, socialLinks: { ...config.socialLinks, facebook: value } })} /><SocialInput icon={Twitter} label="Twitter/X" value={config.socialLinks?.twitter || ''} onChange={value => setConfig({ ...config, socialLinks: { ...config.socialLinks, twitter: value } })} /><SocialInput icon={Linkedin} label="LinkedIn" value={config.socialLinks?.linkedin || ''} onChange={value => setConfig({ ...config, socialLinks: { ...config.socialLinks, linkedin: value } })} /></div></div>
    <section aria-labelledby="agency-payments" className="space-y-5 rounded-3xl border bg-zinc-50/60 p-6">
      <h3 id="agency-payments" className="text-lg font-bold">Pagamentos</h3>
      <p className="text-sm text-zinc-500">Campos opcionais. Configure os dados de cobrança que seus clientes poderão consultar.</p>
