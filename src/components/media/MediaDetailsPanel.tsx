...
interface MediaDetailsPanelProps { asset: MediaAsset; brandName: string; canManage?: boolean; downloading?: boolean; onClose: () => void; onDownload?: (asset: MediaAsset) => void; onDelete: (asset: MediaAsset) => void; onRestore: (asset: MediaAsset) => void; }
const formatSize = (bytes: number) => bytes < 1024 * 1024 ? `${Math.max(1, Math.round(bytes / 1024))} KB` : `${(bytes / (1024 * 1024)).toFixed(2)} MB`;

export default function MediaDetailsPanel({ asset, brandName, canManage = true, downloading = false, onClose, onDownload, onDelete, onRestore }: MediaDetailsPanelProps) {
  ...
  return <div className="max-h-[70vh] overflow-y-auto pr-1">
    ...
    {downloadUrl && <div className="mt-6 flex flex-wrap gap-2"><a href={downloadUrl} target="_blank" rel="noreferrer" className="min-h-11 rounded-xl border border-zinc-300 px-4 py-3 text-sm font-bold hover:bg-zinc-50"><ExternalLink className="mr-1 inline h-4 w-4" />Abrir</a>{onDownload && <button type="button" disabled={downloading} onClick={() => onDownload(asset)} className="min-h-11 rounded-xl border border-zinc-300 px-4 py-3 text-sm font-bold disabled:opacity-50 hover:bg-zinc-50"><Download className="mr-1 inline h-4 w-4" />{downloading ? 'Baixando...' : 'Baixar'}</button>}</div>}
    ...
    <div className="mt-8 flex flex-wrap justify-end gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-xl border border-zinc-300 px-5 text-sm font-bold hover:bg-zinc-50">Fechar</button>{canManage && asset.status === 'ready' && <button type="button" onClick={() => onDelete(asset)} className="min-h-11 rounded-xl bg-red-600 px-5 text-sm font-bold text-white hover:bg-red-700">Mover para lixeira</button>}{canManage && asset.status === 'deleted' && <button type="button" onClick={() => onRestore(asset)} className="min-h-11 rounded-xl bg-black px-5 text-sm font-bold text-white hover:bg-zinc-800">Restaurar</button>}</div>
  </div>;
}
