...
export default function PostMediaCarousel({ post }: { post: Post }) {
  ...
  return <div className="relative h-full w-full overflow-hidden bg-zinc-100">
    ...
    {safeUrl && <a href={safeUrl} download={current.name} target="_blank" rel="noreferrer" aria-label={`Baixar ${current.name}`} className="absolute right-3 top-3 rounded-full bg-black/70 p-3 text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"><Download className="h-4 w-4" /></a>}
  </div>;
}
