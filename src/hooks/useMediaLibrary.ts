...
export function useMediaLibrary(initialBrandId?: string | null) {
  ...
  useEffect(() => {
    ...
    listMediaPage({ brandId: filters.brandId, type: filters.type, status: filters.status, order: filters.order, cursor, pageSize: MEDIA_LIBRARY_PAGE_SIZE })
    ...
    return () => { active = false; };
  }, [cursor, filters.brandId, filters.order, filters.status, filters.type, refreshKey]);
