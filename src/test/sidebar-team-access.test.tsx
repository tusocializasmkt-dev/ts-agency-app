...
describe('menu por perfil', () => {
  ...
  it('cliente vê Mídias e não vê itens administrativos', () => { authState.role = 'client'; render(<MemoryRouter><Sidebar /></MemoryRouter>); expect(screen.getByText('Mídias')).toBeInTheDocument(); expect(screen.getByText('Minha Empresa')).toBeInTheDocument(); expect(screen.queryByText('Equipe')).not.toBeInTheDocument(); expect(screen.queryByText('Clientes')).not.toBeInTheDocument(); expect(screen.queryByText('Lixeira')).not.toBeInTheDocument(); });
});
