...
describe('MediaUploadPanel', () => {
  ...
  it('confirma troca de marca com fila', async () => { const upload = uploadState([item('uploading')]); mocks.useMediaUpload.mockReturnValue(upload); render(<MediaUploadPanel />); const brand = screen.getByLabelText('Cliente'); fireEvent.change(brand, { target: { value: 'b' } }); await waitFor(() => expect(mocks.confirm).toHaveBeenCalled()); await waitFor(() => expect(brand).toHaveValue('b')); expect(upload.clearAll).toHaveBeenCalled(); });
});
