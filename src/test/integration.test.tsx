...
describe('fluxos integrados simplificados', () => {
  ...
  it('cliente vê sidebar e é bloqueado na rota admin', () => { auth.mockReturnValue(authState('client')); const first = Flow({ path: '/cliente' }); expect(screen.getByText('Área cliente')).toBeInTheDocument(); expect(screen.getByRole('navigation')).toHaveTextContent('Minha Empresa'); first.unmount(); Flow({ path: '/admin' }); expect(screen.getByText('Área cliente')).toBeInTheDocument(); });
