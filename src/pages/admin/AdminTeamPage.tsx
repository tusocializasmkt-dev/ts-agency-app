...
import TeamMemberDialog, { type TeamMemberForm } from '../../components/Admin/TeamMemberDialog';
import PasswordDialog from '../../components/auth/PasswordDialog';
import { callCreateTeamMember, callResetTeamMemberPassword, callUpdateTeamMember } from '../../data/functions';
...
export default function AdminTeamPage() {
  ...
  const save = async (data: TeamMemberForm) => {
    ...
    try {
      ...
      else await callCreateTeamMember(data);
      feedback.success(editing ? 'Membro atualizado.' : 'Membro criado.');
  ...
  return <div className="min-w-0 space-y-7">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div><h1 className="text-3xl font-bold">Equipe</h1><p className="text-zinc-500">Gerencie funções e clientes permitidos.</p></div>
      <button onClick={() => setEditing(null)} className="min-h-11 self-start rounded-xl bg-black px-5 py-3 font-bold text-white"><Plus className="mr-2 inline h-4 w-4" />Novo membro</button>
    </header>
    ...
    {reset && <PasswordDialog title={`Redefinir senha de ${reset.displayName}`} processing={processing} onClose={() => setReset(null)} onConfirm={async password => { setProcessing(true); try { await callResetTeamMemberPassword(reset.uid, password); feedback.success('Senha atualizada.'); setReset(null); } finally { setProcessing(false); } }} />}
  </div>;
}
