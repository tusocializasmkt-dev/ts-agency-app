...
export default function AdminClientsPage() {
  ...
  return <div className="space-y-8"><header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-2xl font-bold tracking-tight">Meus Clientes</h2>{isAdmin && <button onClick={() => setDialog(true)} className="min-h-11 rounded-xl bg-black px-5 font-bold text-white"><Plus className="mr-2 inline h-4 w-4" />Novo cliente</button>}</header><ClientList brands={brands} onSelectBrand={id => navigate(`${ROUTES.admin.posts}?brandId=${encodeURIComponent(id)}`)} onManageBrand={isAdmin ? id => navigate(ROUTES.admin.clientDetailFor(id)) : undefined} />{isAdmin && dialog && <ClientDialog processing={creating} onClose={() => setDialog(false)} onSave={save} />}{temporaryAccess && <AccessConfirmationDialog access={temporaryAccess} onClose={() => { setTemporaryAccess(null); setDialog(false); navigate(ROUTES.admin.clientDetailFor(createdId)); }} />}</div>;
}
