...
export function assertInvoiceTransition(current: InvoiceStatus, next: InvoiceStatus) { const allowed: Record<InvoiceStatus, InvoiceStatus[]> = { payment_reported: ['paid', 'suspended', 'cancelled'], pending: ['paid', 'suspended', 'cancelled'], overdue: ['paid', 'suspended', 'cancelled'], suspended: ['pending', 'cancelled'], paid: [], cancelled: [] }; if (!allowed[current].includes(next)) throw new InvoiceError('invalid-transition'); }
