...
test('bloqueio desabilita usuário e revoga tokens sem alterar o status comercial', async () => { await setClientAccessStatus('brand-a', false, dependencies()); assert.deepEqual(calls.disabled, [['brand-a', true]]); assert.deepEqual(calls.revoked, ['brand-a']); assert.equal(calls.brands[0][1].accessEnabled, false); assert.equal('status' in calls.brands[0][1], false); });
